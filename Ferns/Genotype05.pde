...
class Genotype05 implements Genotype
{
	int m_direction = 1;
	color m_baseColor = #008000;

	Genotype05() {}	// Default constructor, for default genotype
	Genotype05(boolean bReverse, color baseColor)
	{
	...
	float GetBaseLength(Object seg)
	{
		...
		{
		case 'T': return segment.GetLength() * 0.80;	// For branch
	...
	float GetLength(Object seg)
	{
		...
		switch (segment.GetType())
		{
		case 'T': return segment.GetLength() * 0.80;	// For trunk
		case 'B': return segment.GetLength() * 0.95;	// For branch
		default:  return 0;
		}
	}
	...
	float GetAbsoluteAngle(Object seg, boolean bIsLeft)
	{
		...
		case 'B': return segment.GetAbsoluteAngle() + (bIsLeft ? -10.0 : +20.0);	// For leaf
		default:  return 0;
		}
	}
	...
	color GetColor(Object seg)
	{
		...
		case 'B': return color(0, green(segment.GetColor()) + 4, blue(segment.GetColor()) + 4);	// For branch
		default:  return 0;
		}
	}
