...
class Genotype01 implements Genotype
{
	int m_direction = 1;
	color m_baseColor = #008000;

	Genotype01() {}	// Default constructor, for default genotype
	Genotype01(boolean bReverse, color baseColor)
	{
	...
	float GetBaseLength(Object seg)
	{
		...
		case 'B': return segment.GetLength() * 1.25;	// For leaf
		default:  return 0;
		}
	}
	float GetLength(Object seg)
	{
		...
		case 'B': return segment.GetLength() * 0.90;	// For branch
		default:  return 0;
		}
	}
	float GetBaseWidth(Object seg)
	{
		if (seg == null) return CANVAS_WIDTH / 50.0;	// For trunk
		Segment segment = (Segment)seg;
		switch (segment.GetType())
		{
		case 'T': return segment.GetWidth() * 0.95;	// For branch
		case 'B': return segment.GetWidth() * 0.5;	// For leaf
		default:  return 0;
		}
	}
	float GetWidth(Object seg)
	{
		...
		switch (segment.GetType())
		{
		case 'T': return segment.GetWidth() * 0.95;	// For trunk
		case 'B': return segment.GetWidth() * 0.70;	// For branch
		default:  return 0;
		}
	}
	float GetAbsoluteAngle(Object seg, boolean bIsLeft)
	{
		...
		case 'B': return segment.GetAbsoluteAngle() + (bIsLeft ? -22.0 : +22.0);	// For leaf
		default:  return 0;
		}
	}
	float GetAngle(Object seg, boolean bIsLeft)
	{
		...
		switch (segment.GetType())
		{
		case 'T': return m_direction * 3.0;	// For trunk
		case 'B': return m_direction * (bIsLeft ? -2.0 : +2.0);	// For branch
		default:  return 0;
		}
	}
