...
class Genotype03 implements Genotype
{
	int m_direction = 1;
	color m_baseColor = #004020;

	Genotype03() {}	// Default constructor, for default genotype
	Genotype03(boolean bReverse, color baseColor)
	{
	...
	{
		if (seg == null) return m_direction * 16.0;	// For trunk
		Segment segment = (Segment)seg;
		switch (segment.GetType())
		{
		case 'T': return segment.GetAbsoluteAngle() + (bIsLeft ? -45.0 : +45.0);	// For branch
		case 'B': return segment.GetAbsoluteAngle() + (bIsLeft ? -45.0 : +45.0);	// For leaf
		default:  return 0;
		}
	}
	...
	color GetColor(Object seg)
	{
		...
		switch (segment.GetType())
		{
		case 'T': return color(0, green(segment.GetColor()) + 5, blue(segment.GetColor()) + 8);	// For trunk
		case 'B': return color(0, green(segment.GetColor()) + 8, blue(segment.GetColor()) + 10);	// For branch
		default:  return 0;
		}
	}
