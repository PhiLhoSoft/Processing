...
class Fern
{
	...
	Fern(GeomPoint position, Genotype genotype, float startTime, float endTime)
	{
		m_position = position;
		m_genotype = genotype;
	...
	//~ int i = 0;
	private GeomPoint GetFernPosition()
	{
...
//~ 		println(xPos);
		return new GeomPoint(xPos, ROOT_POS_Y);
	}

	GeomPoint GetPosition() { return m_position; }
...
//~ 	noLoop();	// No animation
  ...
  ProcessingWrapper.Register(this);
