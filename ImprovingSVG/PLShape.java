...
public class PLShape implements PConstants {
  ...
  /**
   * The width of the PLShape document.
   * @webref
  ...
  protected PLShape parent;
  protected int childCount;
  protected PLShape[] children;
...
//  }
  ...
  */
  ...
  protected void styles(PGraphics g) {
    ...
    if (fill.draw) {
      //System.out.println("filling " + PApplet.hex(fill.color));
      g.fill(fill.color);
    } else {
      g.noFill();
    }
  ...
  protected void drawPath(PGraphics g) {
    ...
    for (int j = 0; j < childCount; j++) {
      ...
      for (int i = 0; i < count; i++) {
        if (style) {
          if (children[j].fill.draw) {
            g.fill(vert[i][R], vert[i][G], vert[i][B]);
          } else {
            g.noFill();
          }
          if (children[j].stroke.draw) {
...
PApplet.println("-> Path");
   ...
   */
  public PLShape getChild(int index) {
    return children[index];
  }

  /**
   * Extracts a child shape from a parent shape. Specify the name of the shape with the <b>target</b> parameter.
   * The shape is returned as a <b>PLShape</b> object, or <b>null</b> is returned if there is an error.
   * @param target the name of the shape to get
   * @webref
   * @brief Returns a child element of a shape as a PLShape object
   */
  public PLShape getChild(String target) {
    if (name != null && name.equals(target)) {
      return this;
    }
    if (nameTable != null) {
      PLShape found = nameTable.get(target);
      if (found != null) return found;
    }
    for (int i = 0; i < childCount; i++) {
      PLShape found = children[i].getChild(target);
      if (found != null) return found;
    }
    return null;
  }
   ...
   */
  public PLShape findChild(String target) {
    if (parent == null) {
  ...
  public void addChild(PLShape who) {
    if (children == null) {
      children = new PLShape[1];
    }
    if (childCount == children.length) {
      children = (PLShape[]) PApplet.expand(children);
    }
   ...
   */
  protected void addName(String nom, PLShape shape) {
    if (parent != null) {
      parent.addName(nom, shape);
    } else {
      if (nameTable == null) {
        nameTable = new HashMap<String,PLShape>();
      }
...
//  public PLShape createGroup() {
//    PLShape group = new PLShape();
//    group.kind = GROUP;
//    addChild(group);
//    return group;
//  }
